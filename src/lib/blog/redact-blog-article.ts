...
const INTERNAL_LINKS = [
  ...
  `${SITE_URL}/es/servicios/accidentes-trafico`,
  `${SITE_URL}/es/servicios/derecho-familia`,
  `${SITE_URL}/es/servicios/negligencias-medicas`,
  `${SITE_URL}/es/servicios/permisos-residencia`,
  `${SITE_URL}/es/servicios/responsabilidad-civil`,
  `${SITE_URL}/es/servicios/responsabilidad-administracion`,
  `${SITE_URL}/es/blog`,
...
type CategoryRel = { name_es: string | null; slug_es: string | null } | null;
...
async function loadPost(
  ...
  let query = supabase
    .from("blog_posts")
    .select(
      `id, title_es, slug_es, excerpt_es, content_es, meta_description_es, reading_time_minutes, tags_es,
       blog_categories(name_es, slug_es)`
    );

  if (input.postId) {
    query = query.eq("id", input.postId);
  } else if (input.slug) {
    query = query.eq("slug_es", input.slug);
  } else if (input.articleUrl) {
    const parsed = parseArticleUrl(input.articleUrl);
    query = parsed.locale === "en" ? query.eq("slug_en", parsed.slug) : query.eq("slug_es", parsed.slug);
  } else {
  ...
  const row = data as Omit<BlogPostRow, "blog_categories"> & {
    blog_categories?: CategoryRel | CategoryRel[];
  };
  return {
    ...row,
    blog_categories: asSingleRelation(row.blog_categories),
  };
}
...
const SEO_FIELDS_SYSTEM_PROMPT = `Genera metadatos SEO en español para un artículo del blog de GVC Abogados (despacho de Murcia: accidentes, familia, divorcio, negligencias médicas, extranjería, responsabilidad civil y de la Administración).
...
- meta_description: meta description (140-155 caracteres). Debe ser única, incluir el tema y un CTA suave (consulta, derechos, Murcia). No copies el excerpt.
- meta_keywords: hasta 10 keywords separadas por coma (abogados Murcia, el área del artículo, trámite o norma).
...
- Tono de despacho, no de reclamo.`;
...
async function generateSeoFields(
  ...
  const defaultKeywords =
    "abogados Murcia, GVC Abogados, derecho, consulta legal Murcia";
...
export async function redactBlogArticle(input: RedactBlogArticleInput): Promise<RedactBlogArticleResult> {
  ...
  console.log(`[BLOG-REDACTOR] Artículo: ${post.title_es}`);
  console.log(
    `[BLOG-REDACTOR] Modelo: ${model} | Web Search nativo | temperature: ${isGpt5Model(model) ? "default (gpt-5.x)" : temperature}`
  );

  if (input.seoOnly) {
    const html = post.content_es?.trim();
    if (!html) throw new Error("El artículo no tiene contenido HTML para generar SEO");

    const seo = await generateSeoFields(openai, post.title_es, html, model, temperature);
    const wordCount = countWords(html);
    const readingTime = post.reading_time_minutes || estimateReadingTimeMinutes(wordCount);
    const payload = {
      excerpt_es: clampSeoText(seo.excerpt, 300),
      meta_description_es: clampSeoText(seo.metaDescription, 155),
      tags_es: seo.metaKeywords,
    ...
    return {
      ...
      wordCount,
      readingTime,
      excerpt: payload.excerpt_es,
      metaDescription: payload.meta_description_es,
  ...
  const briefing = [
    `TITULO DEL ARTICULO:\n${post.title_es}`,
    `CATEGORIA: ${post.blog_categories?.name_es || "Blog"} (${post.blog_categories?.slug_es || ""})`,
    internalLinksBriefing(),
    "Usa web_search para contrastar normativa oficial de España/Murcia antes de redactar.",
    "Redacta el artículo completo en HTML. Solo español. No toques la versión inglesa.",
  ].join("\n\n");
  ...
  const refineInput = [
    `TITULO: ${post.title_es}`,
    internalLinksBriefing(),
    "Vuelve a usar web_search para contrastar plazos, baremos y requisitos oficiales.",
    `BORRADOR HTML:\n${draftHtml}`,
  ...
  const content = normalizeBlogArticleHtml(extractHtmlFromModelOutput(finalRaw), post.title_es);
  const wordCount = countWords(content);
  const readingTime = estimateReadingTimeMinutes(wordCount);
  const seo = await generateSeoFields(openai, post.title_es, content, model, temperature);
