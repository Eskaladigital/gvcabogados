...
export const metadata: Metadata = {
  title: 'Mapa del Sitio | GVC Abogados',
  description: 'Navegue por todas las páginas y servicios de García-Valcárcel & Cáceres Abogados en Murcia.',
...
export default async function SitemapPage() {
  ...
  const staticPagesEs = [
    ...
    { href: '/es/sitemap', label: 'Mapa del Sitio' },
  ];
  ...
  const groupsEs: UrlGroup[] = [
    { title: 'Páginas Principales', icon: <Home size={20} className="text-white" />, urls: staticPagesEs },
    { title: 'Servicios (Genéricos)', icon: <Scale size={20} className="text-white" />, urls: genericServicesEs },
    { title: `Servicios por Ciudad (${localPagesEs.length})`, icon: <Scale size={20} className="text-white" />, urls: localPagesEs },
    { title: `Blog (${blogPagesEs.length})`, icon: <BookOpen size={20} className="text-white" />, urls: blogPagesEs },
  ];

  const groupsEn: UrlGroup[] = [
    { title: 'Main Pages (EN)', icon: <Globe size={20} className="text-white" />, urls: staticPagesEn },
    { title: 'Services — Generic (EN)', icon: <Scale size={20} className="text-white" />, urls: genericServicesEn },
    { title: `Services by City (EN) (${localPagesEn.length})`, icon: <Scale size={20} className="text-white" />, urls: localPagesEn },
    { title: `Blog (EN) (${blogPagesEn.length})`, icon: <BookOpen size={20} className="text-white" />, urls: blogPagesEn },
  ];

  const totalUrls = [...groupsEs, ...groupsEn].reduce((sum, g) => sum + g.urls.length, 0);
          ...
          <div className="container-custom">
            <div className="max-w-3xl">
              ...
              <p className="text-base text-neutral-300 leading-relaxed">
                Todas las páginas y servicios legales de García-Valcárcel &amp; Cáceres Abogados — <strong className="text-white">{totalUrls} URLs</strong> en total.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="container-custom max-w-6xl">
            ...
            <div className="mt-8 bg-brand-brown/10 border border-brand-brown/20 p-6 rounded-xl text-center">
              <p className="text-sm text-brand-dark">
                ...
                <Link href="/es/contacto" className="font-semibold text-brand-brown hover:underline">
                  Contáctenos
                </Link>
              </p>
            </div>
    ...
    <div className="bg-white p-8 rounded-xl shadow-sm border border-neutral-200">
      ...
      <div className="mt-4 pt-3 border-t border-neutral-200">
        <p className="text-xs text-neutral-500">
          {group.urls.length} {group.urls.length === 1 ? 'página' : 'páginas'}
        </p>
      </div>
    </div>
  );
