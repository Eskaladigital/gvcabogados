...
import {
  Shield, Scale, Phone, MapPin, CheckCircle2,
  ArrowRight, Clock, FileText, ShieldCheck
} from 'lucide-react';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = content.titleEs || `Abogados de Responsabilidad Civil en ${cityName} — GVC Abogados`;
  const description = content.metaDescriptionEs || content.shortDescriptionEs ||
    `Abogados especialistas en responsabilidad civil y seguros en ${cityName}. Indemnizaciones por daños, defensa y reclamaciones. Más de 55 años de experiencia. ☎ 968 241 025.`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: { en: `/en/services/civil-liability/${params.ciudad}` },
    },
...
const DEFAULT_STATS = [
  ...
  { value: 'Art. 1902 CC', label: 'Código Civil' },
  { value: 'Sin compromiso', label: 'Primera consulta' },
];

const DEFAULT_TIPOS_RESPONSABILIDAD = [
  { titulo: 'Responsabilidad contractual', descripcion: 'Incumplimiento de obligaciones pactadas en contratos civiles y mercantiles. Reclamación de daños y perjuicios derivados.' },
  { titulo: 'Responsabilidad extracontractual', descripcion: 'Daños causados sin relación contractual previa, regulados por el Art. 1902 del Código Civil.' },
  { titulo: 'Responsabilidad profesional', descripcion: 'Negligencias de profesionales (médicos, arquitectos, abogados) en el ejercicio de su actividad.' },
  { titulo: 'Responsabilidad por productos', descripcion: 'Daños causados por productos defectuosos conforme a la Ley General de Defensa de Consumidores.' },
  { titulo: 'Responsabilidad por vecindad', descripcion: 'Daños entre propiedades colindantes: filtraciones, humedades, inmisiones y desperfectos estructurales.' },
  { titulo: 'Comunidades de propietarios', descripcion: 'Reclamaciones por daños en elementos comunes, defectos constructivos y siniestros en comunidades.' },
];

const DEFAULT_PLAZOS_PRESCRIPCION = [
  { tipo: 'Extracontractual', plazo: '1 año (Art. 1968.2 CC)', descripcion: 'Plazo desde que el perjudicado conoce el daño. Se interrumpe con reclamación extrajudicial o judicial.' },
  { tipo: 'Contractual', plazo: '5 años (Art. 1964 CC)', descripcion: 'Plazo general para acciones personales sin plazo especial. Aplicable a la mayoría de incumplimientos contractuales.' },
  { tipo: 'Productos defectuosos', plazo: '3 años (Art. 143 TRLGDCU)', descripcion: 'Desde que el perjudicado sufrió el daño o desde que tuvo conocimiento del defecto del producto.' },
  { tipo: 'Seguro directo', plazo: '1 año (Art. 23 LCS)', descripcion: 'Acción directa del perjudicado contra la aseguradora del responsable. Desde la fecha del siniestro.' },
];

export default async function ResponsabilidadCivilLocalPage({ params }: Props) {
  const content = await getServiceContentByServiceAndCity(SERVICE_KEY, params.ciudad);
  if (!content) notFound();
  ...
  const tiposResponsabilidad = (custom.tipos_responsabilidad as typeof DEFAULT_TIPOS_RESPONSABILIDAD) || DEFAULT_TIPOS_RESPONSABILIDAD;
  const plazosPrescripcion = (custom.plazos_prescripcion as typeof DEFAULT_PLAZOS_PRESCRIPCION) || DEFAULT_PLAZOS_PRESCRIPCION;
  ...
  return (
    <>
      ...
      <ServiceSchema
        name={content.titleEs || `Abogados de Responsabilidad Civil en ${cityName}`}
      ...
      <main>
        ...
        <section className="bg-[#1a1a1a] relative overflow-hidden min-h-[75vh] md:min-h-[80vh] flex flex-col">
          <div className="absolute inset-0 opacity-25 z-0">
            <Image
              ...
              alt={`Abogados de responsabilidad civil en ${cityName}`}
              fill
          ...
          <div className="container-custom relative z-[2] flex-1 flex items-center py-14 md:py-20">
            <div className="max-w-[720px] w-full">
              ...
              <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-[3.5rem] font-bold text-white leading-[1.15] mb-5 md:mb-6">
                Abogados de{' '}
                <em className="italic text-brand-gold font-normal">responsabilidad civil</em>{' '}
                en {cityName}
              </h1>

              <p className="text-base md:text-lg text-neutral-300 leading-relaxed mb-8 md:mb-10 max-w-[580px]">
                {content.shortDescriptionEs || `Especialistas en responsabilidad civil y seguros en ${cityName}. Reclamaciones por daños, defensa frente a aseguradoras y máxima indemnización.`}
              </p>
          ...
          <div className="container-custom max-w-6xl">
            <div className="reveal">
              ...
              <h2 className="section-title mb-6">
                {content.titleEs || `Abogados de responsabilidad civil en ${cityName}`}
              </h2>
              ...
              <div className="relative float-right ml-8 mb-6 w-full sm:w-[320px] lg:w-[380px]">
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-xl">
                  <Image
                    src="/images/slides/garcia_valcarcel_caceres_abogados_slide_home_v2.webp"
                    alt={`Abogados responsabilidad civil ${cityName}`}
                    fill
                    className="object-cover"
          ...
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                ...
                <h2 className="section-title mb-4">Tipos de responsabilidad civil en {cityName}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {tiposResponsabilidad.map((tipo, i) => (
                  <div key={i} className="reveal group bg-white p-7 rounded-2xl border border-neutral-100 hover:border-brand-brown/30 hover:shadow-lg transition-all text-center">
                    <div className="w-14 h-14 mx-auto mb-5 bg-brand-brown/10 rounded-xl flex items-center justify-center group-hover:bg-brand-brown transition-all">
                      <Shield size={24} className="text-brand-brown group-hover:text-white transition-colors" />
                    </div>
          ...
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                ...
                <h2 className="section-title mb-4">Responsabilidad civil en {cityName}: lo que debe saber</h2>
              </div>
        ...
        {/* ═══════════════════════════════════════
            BANNER: CONCEPTOS INDEMNIZATORIOS
        ═══════════════════════════════════════ */}
            ...
            <div className="reveal flex flex-col md:flex-row items-center gap-8 md:gap-12">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="font-serif text-xl md:text-2xl font-semibold text-white mb-3">
                  Conceptos indemnizatorios en {cityName}
                </h3>
                <p className="text-sm text-neutral-300 leading-relaxed max-w-2xl">
                  La indemnización por responsabilidad civil comprende el <strong className="text-white">daño emergente</strong> (gasto
                  efectivamente producido), el <strong className="text-white">lucro cesante</strong> (beneficio dejado de percibir) y el{' '}
                  <strong className="text-white">daño moral</strong> (sufrimiento y perjuicio personal). Conforme al{' '}
                  <strong className="text-white">Art. 1106 del Código Civil</strong>, el resarcimiento debe ser íntegro.
                </p>
              </div>
        ...
        ═══════════════════════════════════════ */}
        {plazosPrescripcion.length > 0 && (
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" />
                  <span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Plazos legales</span>
                  <span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Plazos de prescripción en responsabilidad civil</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {plazosPrescripcion.map((item, i) => (
                  <div key={i} className="reveal group bg-white p-7 md:p-8 rounded-2xl border border-neutral-100 hover:border-brand-brown/30 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-3">
                      <div className="w-11 h-11 bg-brand-brown/10 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-brand-brown transition-all">
                        <Clock size={20} className="text-brand-brown group-hover:text-white transition-colors" />
                      </div>
                      <div>
                        <h3 className="font-serif text-base font-semibold text-brand-dark">{item.tipo}</h3>
                        <span className="text-sm font-bold text-brand-brown">{item.plazo}</span>
                      </div>
                    </div>
                ...
                <div className="lg:col-span-2">
                  <div className="bg-brand-brown-hover text-white p-8 md:p-10 rounded-2xl sticky top-24">
                    ...
                    <h3 className="font-serif text-xl font-semibold mb-4">
                      ¿Necesita reclamar daños en {cityName}?
                    </h3>
          ...
          <section className="py-16 md:py-24 bg-white">
            <div className="container-custom max-w-4xl">
              <div className="reveal text-center mb-12">
                ...
                <h2 className="section-title mb-4">Preguntas frecuentes sobre responsabilidad civil en {cityName}</h2>
              </div>
          ...
          <div className="container-custom max-w-4xl">
            <div className="reveal text-center">
              <p className="text-sm text-neutral-400 mb-4">
                Consulte también nuestra página general sobre responsabilidad civil con información
                sobre tipos de reclamaciones, plazos de prescripción y el proceso completo.
              </p>
              <Link
              ...
              >
                Ver página general de Responsabilidad Civil <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
          ...
          <div className="container-custom max-w-4xl relative z-10">
            <div className="reveal text-center">
              ...
              <h2 className="section-title-white mb-6">
                Abogados de responsabilidad civil en {cityName}
              </h2>
