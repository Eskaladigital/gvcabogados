...
import {
  Landmark, Scale, Phone, MapPin, CheckCircle2,
...
export async function generateStaticParams() {
  ...
  const { data: nuevo } = await supabaseAdmin
    .from('svc_responsabilidad_admin')
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = content.titleEs || `Responsabilidad de la Administración en ${cityName} — GVC Abogados`;
  const description = content.metaDescriptionEs || content.shortDescriptionEs ||
    `Abogados especialistas en responsabilidad patrimonial de la Administración en ${cityName}. Reclamaciones por mal funcionamiento de servicios públicos. Más de 55 años de experiencia. ☎ 968 241 025.`;
...
const DEFAULT_STATS = [
  ...
  { value: 'Ley 40/2015', label: 'LRJSP actualizada' },
  { value: 'Sin compromiso', label: 'Primera consulta' },
];

const DEFAULT_TIPOS_RESPONSABILIDAD = [
  { titulo: 'Sanitaria', descripcion: 'Errores en hospitales y centros de salud públicos, listas de espera con daño y diagnósticos tardíos.' },
  { titulo: 'Urbanística', descripcion: 'Daños derivados de la planificación urbanística, licencias ilegales o inactividad municipal.' },
  { titulo: 'Vía pública', descripcion: 'Caídas, accidentes por mal estado de aceras, carreteras, alumbrado o señalización deficiente.' },
  { titulo: 'Servicios públicos', descripcion: 'Funcionamiento anormal de servicios como transporte, recogida de residuos o abastecimiento de agua.' },
  { titulo: 'Centros penitenciarios', descripcion: 'Lesiones, fallecimientos o daños sufridos por internos bajo custodia de la Administración.' },
  { titulo: 'Educativa', descripcion: 'Accidentes en centros escolares públicos, negligencias en la vigilancia de menores.' },
];
...
export default async function ResponsabilidadAdministracionLocalPage({ params }: Props) {
  ...
  const organismos = (custom.organismos as typeof DEFAULT_ORGANISMOS) || DEFAULT_ORGANISMOS;
  ...
  return (
    <>
      ...
      <ServiceSchema
        name={content.titleEs || `Responsabilidad de la Administración en ${cityName}`}
        ...
        <section className="bg-[#1a1a1a] relative overflow-hidden min-h-[75vh] md:min-h-[80vh] flex flex-col">
          <div className="absolute inset-0 opacity-25 z-0">
            <Image
              ...
              alt={`Abogados responsabilidad de la administración en ${cityName}`}
              fill
          ...
          <div className="container-custom relative z-[2] flex-1 flex items-center py-14 md:py-20">
            <div className="max-w-[720px] w-full">
              ...
              <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-[3.5rem] font-bold text-white leading-[1.15] mb-5 md:mb-6">
                ...
                <em className="italic text-brand-gold font-normal">Administración</em>{' '}
                en {cityName}
              </h1>

              <p className="text-base md:text-lg text-neutral-300 leading-relaxed mb-8 md:mb-10 max-w-[580px]">
                {content.shortDescriptionEs || `Abogados especialistas en reclamaciones de responsabilidad patrimonial de la Administración en ${cityName}. Defendemos sus derechos frente a los organismos públicos.`}
              </p>
          ...
          <div className="container-custom max-w-6xl">
            <div className="reveal">
              ...
              <h2 className="section-title mb-6">
                {content.titleEs || `Responsabilidad patrimonial de la Administración en ${cityName}`}
              </h2>
              ...
              <div className="relative float-right ml-8 mb-6 w-full sm:w-[320px] lg:w-[380px]">
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-xl">
                  <Image
                    src="/images/slides/garcia_valcarcel_caceres_abogados_slide_home_v2.webp"
                    alt={`Abogados responsabilidad administración ${cityName}`}
                    fill
                    className="object-cover"
            ...
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" />
                  <span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Ámbitos de actuación</span>
                  <span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Tipos de responsabilidad patrimonial en {cityName}</h2>
              </div>
                ...
                {tiposResponsabilidad.map((tipo, i) => (
                  <div key={i} className="reveal group bg-white p-7 rounded-2xl border border-neutral-100 hover:border-brand-brown/30 hover:shadow-lg transition-all text-center">
                    <div className="w-14 h-14 mx-auto mb-5 bg-brand-brown/10 rounded-xl flex items-center justify-center group-hover:bg-brand-brown transition-all">
                      <Landmark size={24} className="text-brand-brown group-hover:text-white transition-colors" />
                    </div>
          ...
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                ...
                <h2 className="section-title mb-4">Responsabilidad de la Administración en {cityName}: lo que debe saber</h2>
              </div>
            ...
            <div className="reveal flex flex-col md:flex-row items-center gap-8 md:gap-12">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="font-serif text-xl md:text-2xl font-semibold text-white mb-3">
                  Plazos de reclamación patrimonial en {cityName}
                </h3>
                <p className="text-sm text-neutral-300 leading-relaxed max-w-2xl">
                  El plazo para reclamar responsabilidad patrimonial es de <strong className="text-white">1 año</strong> desde
                  que se produce el daño o se conocen sus efectos (<strong className="text-white">Ley 40/2015 LRJSP</strong>).
                  Si la Administración desestima su reclamación, se abre la vía del recurso contencioso-administrativo
                  ante los juzgados correspondientes. No deje pasar el plazo.
                </p>
              </div>
          ...
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" />
                  <span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Organismos en {cityName}</span>
                  <span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Organismos públicos locales en {cityName}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  ...
                  <div key={i} className="reveal group bg-white p-7 rounded-2xl border border-neutral-100 hover:border-brand-brown/30 hover:shadow-lg transition-all">
                    <div className="flex items-start gap-4 mb-3">
                      <div className="w-11 h-11 bg-brand-brown/10 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-brand-brown transition-all">
                        <Building2 size={20} className="text-brand-brown group-hover:text-white transition-colors" />
                      </div>
                      <div>
                        <h3 className="font-serif text-base font-semibold text-brand-dark">{org.nombre}</h3>
                        <span className="text-[0.65rem] text-brand-brown uppercase tracking-wider">{org.tipo}</span>
                      </div>
                    </div>
                    {org.direccion && (
                      <p className="text-[0.8rem] text-neutral-400 leading-relaxed pl-[60px]">{org.direccion}</p>
                    )}
                  </div>
                ))}
              </div>
                ...
                <div className="lg:col-span-2">
                  <div className="bg-brand-brown-hover text-white p-8 md:p-10 rounded-2xl sticky top-24">
                    ...
                    <h3 className="font-serif text-xl font-semibold mb-4">
                      ¿Ha sufrido daños por la Administración en {cityName}?
                    </h3>
          ...
          <section className="py-16 md:py-24 bg-white">
            <div className="container-custom max-w-4xl">
              <div className="reveal text-center mb-12">
                ...
                <h2 className="section-title mb-4">Preguntas frecuentes sobre responsabilidad de la Administración en {cityName}</h2>
              </div>
          ...
          <div className="container-custom max-w-4xl">
            <div className="reveal text-center">
              <p className="text-sm text-neutral-400 mb-4">
                Consulte también nuestra página general sobre responsabilidad de la Administración con información
                sobre los tipos de reclamación, plazos y el procedimiento completo.
              </p>
              <Link
              ...
              >
                Ver página general de {SERVICE_NAME} <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
          ...
          <div className="container-custom max-w-4xl relative z-10">
            <div className="reveal text-center">
              ...
              <h2 className="section-title-white mb-6">
                Responsabilidad de la Administración en {cityName}
              </h2>
