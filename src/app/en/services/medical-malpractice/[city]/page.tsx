...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = content.titleEn || `Medical Malpractice Lawyers in ${cityName} — GVC Lawyers`;
  const description = content.metaDescriptionEn || content.shortDescriptionEn ||
    `Specialist medical malpractice lawyers in ${cityName}. Diagnostic errors, surgical negligence, A&E failures. Over 55 years of experience. ☎ 968 241 025.`;
...
const DEFAULT_STATS = [
  ...
  { value: 'Medical experts', label: 'Specialist assessment' },
  { value: 'No obligation', label: 'Initial consultation' },
];

const DEFAULT_TIPOS_NEGLIGENCIA = [
  { titulo: 'Diagnostic errors', descripcion: 'Delayed, incorrect or missed diagnoses that worsen the patient\'s condition.', icon: 'stethoscope' },
  { titulo: 'Surgical negligence', descripcion: 'Errors during procedures: retained instruments, wrong-site surgery, avoidable complications.', icon: 'alert' },
  { titulo: 'A&E and triage failures', descripcion: 'Premature discharge, excessive waiting times or lack of proper emergency care.', icon: 'clock' },
  { titulo: 'Obstetric negligence', descripcion: 'Injuries to newborn or mother during childbirth caused by medical malpractice.', icon: 'hospital' },
  { titulo: 'Medication errors', descripcion: 'Incorrect prescriptions, overdoses, uncontrolled interactions or unchecked allergies.', icon: 'file' },
  { titulo: 'Hospital-acquired infections', descripcion: 'Avoidable nosocomial infections due to failures in hygiene and sterilisation protocols.', icon: 'shield' },
];

const DEFAULT_HOSPITALES: Array<{ nombre: string; tipo: string; direccion: string }> = [];

export default async function MedicalMalpracticeLocalPage({ params }: Props) {
  const content = await getServiceContentByServiceAndCity(SERVICE_KEY, params.city);
  if (!content) notFound();
  ...
  const breadcrumbs = [
    ...
    { name: 'Medical Malpractice', href: `/en/services/${FOLDER_SLUG_EN}` },
  ...
  return (
    <>
      ...
      <ServiceSchema
        name={content.titleEn || `Medical Malpractice Lawyers in ${cityName}`}
      ...
      <main>
        ...
        <section className="bg-[#1a1a1a] relative overflow-hidden min-h-[75vh] md:min-h-[80vh] flex flex-col">
          <div className="absolute inset-0 opacity-25 z-0">
            <Image
              ...
              alt={`Medical malpractice lawyers in ${cityName}`}
              fill className="object-cover" priority sizes="100vw" quality={60}
            />
          </div>
          <div className="absolute inset-0 bg-gradient-to-br from-[rgba(26,26,26,0.7)] via-[rgba(30,30,30,0.4)] to-[rgba(26,26,26,0.6)] z-[1]" />
          <div className="absolute inset-0 opacity-[0.03] z-[1]" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,.5) 1px,transparent 1px), linear-gradient(90deg,rgba(255,255,255,.5) 1px,transparent 1px)', backgroundSize: '80px 80px' }} />
            ...
            <div className="max-w-[720px] w-full">
              ...
              <div className="flex items-center gap-3 mb-6 md:mb-8 mt-6">
                ...
                <span className="text-[0.7rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">
                  <MapPin size={12} className="inline mr-1 -mt-0.5" />{cityName}
                </span>
              </div>
              <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-[3.5rem] font-bold text-white leading-[1.15] mb-5 md:mb-6">
                Medical{' '}<em className="italic text-brand-gold font-normal">malpractice lawyers</em>{' '}in {cityName}
              </h1>
              <p className="text-base md:text-lg text-neutral-300 leading-relaxed mb-8 md:mb-10 max-w-[580px]">
                {content.shortDescriptionEn || `Specialists in medical negligence claims in ${cityName}. Diagnostic errors, surgical negligence, A&E failures and hospital malpractice.`}
              </p>
              <div className="flex gap-3 md:gap-4 items-center flex-wrap">
                <Link href="/en/contact" className="btn-primary">No-obligation initial consultation →</Link>
          ...
          <div className="container-custom max-w-6xl">
            <div className="reveal">
              <div className="flex items-center gap-3 mb-4">
                <span className="w-9 h-0.5 bg-brand-brown" />
                <span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Specialists in {cityName}</span>
              </div>
              <h2 className="section-title mb-6">{content.titleEn || `Medical malpractice lawyers in ${cityName}`}</h2>
              <div className="relative float-right ml-8 mb-6 w-full sm:w-[320px] lg:w-[380px]">
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-xl">
                  <Image src="/images/slides/garcia_valcarcel_caceres_abogados_slide_home_v2.webp" alt={`Medical malpractice lawyers ${cityName}`} fill className="object-cover" sizes="(max-width: 640px) 100vw, 380px" quality={60} />
            ...
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" /><span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Specialities</span><span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Types of medical malpractice in {cityName}</h2>
              </div>
        ...
        {sections.length > 0 && (
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-6xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" /><span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Relevant information</span><span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Medical malpractice in {cityName}: what you need to know</h2>
              </div>
        ...
        {/* BANNER: CLAIM DEADLINES */}
        <section className="py-16 md:py-20 bg-brand-dark relative overflow-hidden">
          <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, rgba(204,178,127,0.5) 0%, transparent 50%), radial-gradient(circle at 80% 50%, rgba(204,178,127,0.3) 0%, transparent 50%)' }} />
          <div className="container-custom max-w-5xl relative z-10">
            <div className="reveal flex flex-col md:flex-row items-center gap-8 md:gap-12">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="font-serif text-xl md:text-2xl font-semibold text-white mb-3">Claim deadlines for medical negligence</h3>
                <p className="text-sm text-neutral-300 leading-relaxed max-w-2xl">
                  Civil liability claims for medical negligence must be filed within <strong className="text-white">1 year</strong> from when the patient becomes aware of the injury.
                  For public healthcare negligence, the administrative claim deadline is also <strong className="text-white">1 year</strong>.
                  Acting promptly is essential to protect your rights.
                </p>
              </div>
              <div className="shrink-0">
                <Link href="/en/contact" className="btn-primary whitespace-nowrap">Check deadlines →</Link>
              </div>
            </div>
          </div>
        </section>
        ...
        {hospitales.length > 0 && (
          <section className="py-16 md:py-24 bg-neutral-50">
            <div className="container-custom max-w-5xl">
              <div className="reveal text-center mb-14">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" /><span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Healthcare centres</span><span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Hospitals and reference centres in {cityName}</h2>
                <p className="text-sm text-neutral-400 max-w-2xl mx-auto">If you have suffered malpractice at any of these centres, we can help with your claim.</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="container-custom max-w-6xl">
              <div className="reveal grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16">
                <div className="lg:col-span-3">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="w-9 h-0.5 bg-brand-brown" /><span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">Our process</span>
                  </div>
                  <h2 className="section-title mb-8">How we handle your claim in {cityName}</h2>
                  <div className="space-y-0">
                    {process.map((step, i) => (
                      <div key={i} className="flex gap-4 group">
                        ...
                        <div className="pb-8"><p className="text-sm text-neutral-500 leading-relaxed pt-2">{step}</p></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="lg:col-span-2">
                  <div className="bg-brand-brown-hover text-white p-8 md:p-10 rounded-2xl sticky top-24">
                    <div className="w-14 h-14 bg-white/10 rounded-xl flex items-center justify-center mb-6"><Phone size={24} className="text-brand-brown" /></div>
                    <h3 className="font-serif text-xl font-semibold mb-4">Suffered medical negligence in {cityName}?</h3>
                    <p className="text-sm text-white/80 leading-relaxed mb-6">We advise you from the very first moment. No-obligation initial consultation.</p>
                    <ul className="space-y-3 mb-8">
                      {['Immediate assessment of your case', `In-person and online service in ${cityName}`, 'Collaborating medical experts'].map((item, i) => (
                        <li key={i} className="flex items-center gap-2 text-sm text-white/90"><CheckCircle2 size={14} className="text-brand-brown shrink-0" />{item}</li>
                      ))}
                    </ul>
                    <div className="flex flex-col gap-3">
                      <a href="tel:+34968241025" className="inline-flex items-center justify-center gap-2 bg-brand-brown text-white text-sm font-semibold px-6 py-4 rounded-xl transition-all hover:bg-brand-brown/80 hover:shadow-xl"><Phone size={18} /> 968 241 025</a>
                      <Link href="/en/contact" className="inline-flex items-center justify-center gap-2 bg-white/10 text-white text-sm font-medium px-6 py-3 rounded-xl border border-white/20 transition-all hover:bg-white/20">Send us a message →</Link>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="reveal">
              <div className="bg-gradient-to-br from-brand-brown to-brand-brown/95 p-10 md:p-14 rounded-2xl">
                <div className="text-center mb-10">
                  <h2 className="font-serif text-2xl md:text-3xl font-semibold text-brand-dark mb-3">Why choose García-Valcárcel & Cáceres in {cityName}?</h2>
                </div>
          ...
          <section className="py-16 md:py-24 bg-white">
            <div className="container-custom max-w-4xl">
              <div className="reveal text-center mb-12">
                <div className="flex items-center gap-3 justify-center mb-4">
                  <span className="w-9 h-0.5 bg-brand-brown" /><span className="text-[0.6rem] font-semibold text-brand-brown tracking-[0.2em] uppercase">We answer your questions</span><span className="w-9 h-0.5 bg-brand-brown" />
                </div>
                <h2 className="section-title mb-4">Frequently asked questions about medical malpractice in {cityName}</h2>
              </div>
        ...
        <section className="py-12 md:py-16 bg-neutral-50 border-t border-neutral-200">
          <div className="container-custom max-w-4xl">
            <div className="reveal text-center">
              <p className="text-sm text-neutral-400 mb-4">Also visit our general medical malpractice page for information on types of claims, deadlines and the full legal process.</p>
              <Link href={`/en/services/${FOLDER_SLUG_EN}`} className="inline-flex items-center gap-2 text-sm font-semibold text-brand-brown hover:text-brand-brown-hover transition-colors">
                View the general Medical Malpractice page <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="py-16 md:py-24 bg-brand-dark relative overflow-hidden">
          <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'radial-gradient(circle at 30% 50%, rgba(204,178,127,0.4) 0%, transparent 50%), radial-gradient(circle at 70% 50%, rgba(204,178,127,0.3) 0%, transparent 50%)' }} />
          <div className="container-custom max-w-4xl relative z-10">
            <div className="reveal text-center">
              <div className="w-20 h-20 bg-brand-brown rounded-2xl flex items-center justify-center mx-auto mb-8 relative">
                <Image src="/images/logo/gvcabogados_murcia_logo_leon_blanco.webp" alt="GVC Lawyers" fill className="object-contain p-4" sizes="80px" />
              </div>
              <h2 className="section-title-white mb-6">Medical malpractice lawyers in {cityName}</h2>
              <p className="text-base md:text-lg text-neutral-300 leading-relaxed mb-4 max-w-2xl mx-auto">Our head office is at <strong className="text-white">Gran Vía, 15 — 3rd Floor, 30008 Murcia</strong>.</p>
              <p className="text-base md:text-lg text-neutral-300 leading-relaxed mb-10 max-w-2xl mx-auto">We serve clients in {cityName} in person and by video conference. No-obligation initial consultation.</p>
              <div className="flex gap-3 md:gap-4 items-center flex-wrap justify-center">
                <Link href="/en/contact" className="inline-flex items-center gap-2 bg-brand-brown text-white text-xs font-semibold px-8 py-4 tracking-wide transition-all duration-300 hover:bg-brand-brown/80 hover:-translate-y-0.5 hover:shadow-xl">No-obligation consultation →</Link>
                <a href="tel:+34968241025" className="btn-outline">☎ 968 241 025</a>
              </div>
            </div>
          </div>
